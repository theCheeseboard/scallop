    ...
    ui(new Ui::PartitionWindow)
{
    ...
    ui->noPartitionTableExplanation->setText(tr("You'll need a partition table on this drive to install %1 on it.").arg(branding.name));

    this->drive = drive;
    driveInfo = DriveInfo::loadDrive(drive);
    if (driveInfo == NULL) { //No partition table found on device.
        bool ok = false;
        QString format = QInputDialog::getItem(parent, tr("Initialize Drive"), tr("No valid partition table was found on this device. You'll need to initialize it first.\n\n"
...
void PartitionWindow::on_pushButton_clicked()
{
    ...
    if (QMessageBox::warning(this, "Applying Changes", "We're about to make changes to your disk. Ensure that the operations are correct, then click OK to apply the changes.\n\n"
                             "It's advisable to make a backup because we're doing some dangerous things here.", QMessageBox::Ok | QMessageBox::Cancel, QMessageBox::Ok) == QMessageBox::Ok) {
